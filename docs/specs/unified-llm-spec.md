...
- [x] OpenAI reasoning models (GPT-5+) return `reasoning_tokens` in `Usage` via the Responses API
...
- [x] **OpenAI**: `Usage.cache_read_tokens` is populated from `usage.input_tokens_details.cached_tokens`
- [x] **Anthropic**: adapter supports prompt caching via `cache_control` blocks
- [x] **Anthropic**: if automatic caching is enabled, the adapter injects `cache_control` breakpoints on the system prompt, tool definitions, and stable conversation prefix using a documented heuristic
