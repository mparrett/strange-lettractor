...
- [x] Exactly one start node (shape=Mdiamond or id matching `start`/`Start`) is required
- [x] Exactly one exit node (shape=Msquare or id matching `exit`/`end`) is required
...
- [x] Pipeline outcome is "success" if all goal_gate nodes reached `SUCCESS` or `PARTIAL_SUCCESS`, "fail" otherwise

### 11.4 Goal Gate Enforcement

- [x] Nodes with `goal_gate=true` are tracked throughout execution
- [x] Before allowing exit via a terminal node, the engine checks all goal gate nodes have status `SUCCESS` or `PARTIAL_SUCCESS`
...
- [x] Question supports types: YES_NO, MULTIPLE_CHOICE, FREEFORM, CONFIRMATION
