...
on `iter-0006-interactive-concurrency`. [Console input investigation](../../console-features.md)
...
Task 5 is complete, but the iteration is not. Task 6 must prove nested cancellation/event ordering, independently exhausted budgets, descendant/restart custom-handler behavior, and captured interrupted-child recovery. The local compiler's separately confirmed silent unfinished-trailing-form bug is recorded in [reader compatibility findings](../../let-go-runtime-issues.md); check test discovery counts as well as failure totals.
