...
description: >-
  The software engineering discipline of designing, constructing, and optimizing
  the code-based environments (harnesses) that execute, verify, and persist AI
  agents.
tags:
  - Infrastructure
  - Theory
...
references:
  - type: website
    ...
    annotation: >-
      Popularized the 'Agent = Model + Harness' equation, defining the harness
      as the steering, safety, and instrumentation environment around the LLM.
  - type: paper
    title: >-
    ...
    annotation: >-
      Defines the 'Code as Agent Harness' (CAH) framework, organizing agent
      infrastructure into interface, mechanism, and scaling layers.
  - type: paper
    ...
    annotation: >-
      Introduces the Self-Harness loop for model-specific harness
      self-improvement, verifying edits using regression gates on held-out
      tasks.
---
> **ASDLC Knowledge Base** | Status: Experimental | [View Online](https://asdlc.io/concepts/harness-engineering)
...
* **The Harness Interface Layer:** Dynamically anchors domain context and exposes standardized tool/resource interfaces (like [Model Context Protocol](../concepts/model-context-protocol.md)).
...
In the Agentic Software Development Life Cycle (ASDLC), harness engineering is the foundational discipline that builds the conveyor belt of the **Software Factory** (see [Agentic SDLC](../concepts/agentic-sdlc.md)). 
...
* Intercepting outputs at [Context Gates](../patterns/context-gates.md) to parse warnings and failures into structured telemetry before the agent reads them.
