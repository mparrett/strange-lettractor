...
description: >-
  Open standard for connecting AI agents to external tools and data. Covers
  MCP's role in agentic development, its limits, and why implementation quality
  matters.
tags:
  - Infrastructure
  - Standards
...
references:
  - type: website
    ...
    annotation: >-
      The authoritative protocol specification defining tools, resources,
      prompts, and the Client-Host-Server architecture.
  - type: website
    ...
    accessed: 2026-04-11T00:00:00.000Z
    annotation: >-
      Strategic priorities for 2026: transport scalability, agent-to-agent
      communication, enterprise auth, and governance maturation via Working
      Groups.
  - type: website
    ...
    accessed: 2026-04-11T00:00:00.000Z
    annotation: >-
      Governance update reflecting MCP's move to the Agentic AI Foundation and
      expansion of the core maintainer group.
  - type: website
    ...
    accessed: 2026-03-02T00:00:00.000Z
    annotation: >-
      Empirical study demonstrating that static context often outperforms active
      tool-based retrieval for framework knowledge, informing the static vs.
      dynamic context heuristic.
  - type: website
    ...
    accessed: 2026-03-02T00:00:00.000Z
    annotation: >-
      Analysis of MCP's role in context engineering alongside security risks
      including tool poisoning and cross-server tool shadowing.
  - type: website
    ...
    accessed: 2026-03-02T00:00:00.000Z
    annotation: >-
      Critical analysis of MCP's security surface including prompt injection via
      untrusted data sources and forth-party exploits.
---
> **ASDLC Knowledge Base** | Status: Live | [View Online](https://asdlc.io/concepts/model-context-protocol)
...
In the ASDLC, MCP serves as the **dynamic context delivery layer**: the complement to static context provided by [AGENTS.md](../practices/agents-md-spec.md) and [Specs](../patterns/the-spec.md). Static files supply stable architectural constraints that don't change between tool calls. MCP connections supply mutable, real-time information that the agent needs at the moment of inference. [Context Gates](../patterns/context-gates.md) mediate between the two, filtering MCP outputs before they enter the agent's reasoning loop.
...
This maps directly to [Context Engineering](../concepts/context-engineering.md). The decision framework:
...
This is [Context Engineering](../concepts/context-engineering.md) applied to infrastructure. The same principles that govern how you structure a spec or an `AGENTS.md` file govern how you build an MCP server.
...
These risks are mitigated in the ASDLC through structural patterns rather than protocol-level fixes. [Context Gates](../patterns/context-gates.md) filter MCP outputs before they enter the reasoning loop. [Workflow as Code](../practices/workflow-as-code.md) wraps MCP tool calls in deterministic, type-safe execution pipelines where tool selection is governed by code, not left to agent reasoning. For vendoring tool schemas to prevent drift, see the [Living Specs practice](../practices/living-specs.md) for the same principle applied to specifications.
...
**Orchestration** — MCP provides tools but does not sequence when they run, how failures are handled, or when human escalation is required. That is the job of [Workflow as Code](../practices/workflow-as-code.md).

**Governance** — MCP has no native mechanism for enforcing policy compliance across tool calls. An agent with access to a production database and a deployment tool has no protocol-level guardrail preventing it from deploying untested code. That is the job of [Context Gates](../patterns/context-gates.md) and the [Agent Constitution](../patterns/agent-constitution.md).
